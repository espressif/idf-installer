...
RELEASES_JSON_PATH = f".{sep}src{sep}Resources{sep}download{sep}releases.json"
INDEX_PATH = f".{sep}src{sep}Resources{sep}download{sep}index.html"
INNO_SETUP_PATH = f".{sep}src{sep}InnoSetup{sep}IdfToolsSetup.iss"


# Environment variables from GitHub Actions
installer_type: str = environ.get('INSTALLER_TYPE', 'offline')                  # espressif-ide, offline, online
installer_size: str = environ.get('INSTALLER_SIZE', '1.5 GB')                  # e.g. '1.69 GB'
idf_version = environ.get('ESP_IDF_VERSION', '5.2.1')                         # e.g. '4.4.7'
ide_version = environ.get('ESPRESSIF_IDE_VERSION', '2.13.69')                   # e.g. '2.13.69'
online_installer_version = environ.get('ONLINE_INSTALLER_VERSION', '2.25')   # e.g. '2.25'


if not idf_version:
    print(f"ERROR: IDF version is not provided")
    raise SystemExit(1)
...
def update_index():
    ...
    try:
        ...
        print(f"Error opening file {INDEX_PATH} - {e}")
        raise SystemExit(1)
    ...
    try:
        ...
        print(f"Error writing file {INDEX_PATH} - {e}")
        raise SystemExit(1)
...
def update_releases_json():
    ...
    try:
        ...
        print(f"Error opening file {RELEASES_JSON_PATH} - {e}")
        raise SystemExit(1)
    ...
    try:
        ...
        print(f"Error writing file {RELEASES_JSON_PATH} - {e}")
        raise SystemExit(1)
...
def update_inno_setup():
    ...
    try:
        ...
        print(f"Error opening file {INNO_SETUP_PATH} - {e}")
        raise SystemExit(1)
    ...
    try:
        ...
        print(f"Error writing file {INNO_SETUP_PATH} - {e}")
        raise SystemExit(1)
...
def main():
    ...
    if ide_version and not re.match(r'(\d+\.\d+\.\d+)', ide_version):
        print(f"ERROR: IDE version is not in correct format (it should be 'X.Y.Z')")
        raise SystemExit(1)
    
    if online_installer_version and not re.match(r'(\d+\.\d+)', online_installer_version):
        print(f"ERROR: Online installer version is not in correct format (it should be 'X.Y')")
        raise SystemExit(1)
    
    if installer_type == 'online' and online_installer_version == '':
        print(f"ERROR: online_installer_version is not provided")
        raise SystemExit(1)
    
    if installer_type == 'espressif-ide' and ide_version == '':
        print(f"ERROR: esp_ide_version or espressif_ide_version is not provided")
        raise SystemExit(1)
